((context) => {
    // ===============================================================
    // 1. CSS SIÊU PHẨM (GLASSMORPHISM + SPOTIFY/APPLE MUSIC STYLE)
    // ===============================================================
    const MY_CSS = `
        ...
        /* Nền chính - Đổi màu chuyển động cực chill */
        ...
        /* Layout 2 phần: Trình phát (Trên) & Danh sách (Dưới) */
        .mz-body { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
        
        /* --- KHU VỰC ĐĨA THAN VÀ ĐIỀU KHIỂN --- */
        ...
        .mz-ctrl-btn.active { color: #00d2ff; opacity: 1;} /* Cho nút lặp/trộn */
        ...
        .mz-track { display: flex; align-items: center; padding: 12px 15px; border-radius: 12px; background: rgba(255,255,255,0.03); cursor: pointer; transition: 0.2s; border: 1px solid transparent;}
        ...
        /* Hiệu ứng Equalizer khi đang Play */
        .mz-eq { display: none; width: 20px; height: 16px; align-items: flex-end; gap: 2px; }
        ...
        .mz-track.active .mz-tk-num { display: none; } /* Ẩn số khi hiện EQ */
    ...
    let allMusic = [];     // Toàn bộ nhạc từ API
    let displayList = [];  // Danh sách nhạc đang hiển thị (sau khi lọc)
    let currentTrackIndex = 0; // Index của bài hát hiện tại trong displayList
    let isPlaying = false;
    let isShuffle = false;
    let isRepeat = false;  // false: không lặp, true: lặp 1 bài

    let audio = new Audio();
    audio.crossOrigin = "anonymous"; // Tránh lỗi CORS nếu có

    // Chuyển đổi giây thành mm:ss
    ...
    const runTool = () => {
        ...
        if (!app) {
            ...
            app.innerHTML = `
                ...
                <div class="mz-body">
                    ...
                    <div class="mz-player-area">
                        ...
                        <div class="mz-info">
                            <h2 class="mz-title" id="mz-title">Đang tải nhạc...</h2>
                    ...
                    <div class="mz-playlist-area">
                        <div class="mz-genres-wrap" id="mz-genres-wrap">
                            ...
                            <div class="mz-genre-pill active" data-genre="all">Tất cả</div>
                        </div>
                        <div class="mz-list" id="mz-list">
                            <div style="text-align:center; padding: 20px; color:#94a3b8; font-size:14px;">Đang kết nối Server...</div>
                        </div>
                    </div>
                </div>
            `;
            ...
            // 3. Khai báo các Node DOM
            const $ = (id) => app.querySelector('#' + id);
            
            $('mz-btn-close').onclick = () => { 
                app.style.display = 'none'; 
                // Có thể tùy chọn: audio.pause(); Nếu muốn ẩn tool vẫn hát thì cmt dòng audio.pause lại.
            };

            // 4. Các hàm Render Giao Diện
            const renderGenres = () => {
                // Lấy các thể loại duy nhất
                const genres = [...new Set(allMusic.map(m => m.genre))];
                let html = `<div class="mz-genre-pill active" data-genre="all">Tất cả</div>`;
                genres.forEach(g => {
                    html += `<div class="mz-genre-pill" data-genre="${g}">${g}</div>`;
                });
                $('mz-genres-wrap').innerHTML = html;

                // Gán sự kiện click lọc
                app.querySelectorAll('.mz-genre-pill').forEach(pill => {
                    pill.onclick = (e) => {
                        ...
                        if (selGenre === 'all') displayList = [...allMusic];
                        else displayList = allMusic.filter(m => m.genre === selGenre);
            ...
            const renderPlaylist = () => {
                ...
                if (displayList.length === 0) {
                    listEl.innerHTML = `<div style="text-align:center; padding:20px; color:#94a3b8;">Không có bài hát nào.</div>`;
                    return;
                }

                let html = '';
                displayList.forEach((track, idx) => {
                    // Check xem bài này có đang được phát không
                    ...
                    html += `
                        <div class="mz-track ${isActive}" data-idx="${idx}">
                            ...
                            <!-- Sóng nhạc Equalizer -->
                            <div class="mz-eq">
                                <div class="mz-eq-bar"></div><div class="mz-eq-bar"></div><div class="mz-eq-bar"></div>
                            </div>
                ...
                // Sự kiện bấm chọn bài trong list
            ...
            const updatePlayState = () => {
                if (isPlaying) {
                    $('btn-play').innerHTML = ICONS.pause;
                    $('mz-disc').classList.add('playing');
                } else {
                    $('btn-play').innerHTML = ICONS.play;
                    $('mz-disc').classList.remove('playing');
                }
            ...
            $('btn-play').onclick = () => {
                if (displayList.length === 0) return;
                if (!audio.src) loadTrack(0); // Bấm play mà chưa có source
            ...
            const playNext = () => {
                if (displayList.length === 0) return;
                if (isShuffle) {
                    currentTrackIndex = Math.floor(Math.random() * displayList.length);
                } else {
                    currentTrackIndex = (currentTrackIndex + 1) % displayList.length;
                }
                loadTrack(currentTrackIndex);
                audio.play();
            };
            ...
            // Kéo thả Slider
            $('mz-slider').addEventListener('input', (e) => {
                $('mz-time-current').innerText = formatTime(e.target.value);
            });
            $('mz-slider').addEventListener('change', (e) => {
            ...
            // Lỗi tải nhạc (Link die)
            audio.addEventListener('error', () => {
                $('mz-title').innerText = "❌ Lỗi: Không thể phát bài này";
                setTimeout(playNext, 2000); // Tự động bỏ qua sau 2s
            });
            ...
            const fetchData = () => {
                ...
                try {
                    const cache = localStorage.getItem('mz_all_music');
                    if (cache) {
                        allMusic = JSON.parse(cache);
                        displayList = [...allMusic];
                        renderGenres();
                        renderPlaylist();
                        loadTrack(0); // Load sẵn bài đầu
                    }
                } catch(e){}

                const payload = { action: 'get_music_list' };
                const processRes = (text) => {
                    try {
                        ...
                        if (json.status === 'success' && json.data.length > 0) {
                            ...
                            renderGenres();
                            renderPlaylist();
                            if(audio.paused && !audio.src) loadTrack(0);
                        } else {
                            $('mz-title').innerText = "Kho nhạc trống";
                            $('mz-genre').innerText = "Vui lòng cập nhật Sheet";
                            $('mz-list').innerHTML = `<div style="text-align:center; padding:20px; color:#94a3b8;">Data rỗng.</div>`;
                        }
                    } catch(e){}
                };

                if (context.GM_xmlhttpRequest) {
                    context.GM_xmlhttpRequest({ method: "POST", url: MUSIC_API_URL, data: JSON.stringify(payload), onload: (res) => processRes(res.responseText) });
                } else {
                    fetch(MUSIC_API_URL, { method: "POST", body: JSON.stringify(payload) })
    ...
    return {
        ...
        bgColor: "#1e1b4b", // Màu tím midnight cho icon ngoài trang chủ
        action: runTool
    };
})
