((context) => {
    ...
    const MY_CSS = `
        ...
        .bc-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 25px; margin-bottom: 20px; }
        .bc-sec-title { font-size: 16px; font-weight: 600; color: #FFD700; margin-bottom: 15px; }
        ...
        .bc-file-label { display: flex; align-items: center; justify-content: center; gap: 10px; width: 100%; padding: 15px; border: 2px dashed rgba(255,255,255,0.2); border-radius: 8px; cursor: pointer; color: #94a3b8; background: rgba(255,255,255,0.02); transition:0.2s;}
        .bc-file-label:hover { border-color:#38bdf8; color:#38bdf8;}
        .bc-preview-grid { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
        .bc-preview-item { width: 70px; height: 70px; border-radius: 8px; object-fit: cover; border: 1px solid rgba(255,255,255,0.2); }

        /* TABS CHO QUẢN LÝ */
        .bc-tabs { display: flex; gap: 10px; margin-bottom: 20px; background: rgba(0,0,0,0.2); padding:5px; border-radius:10px;}
        .bc-tab-btn { flex: 1; padding: 10px; border-radius: 6px; border: none; background: transparent; color: #94a3b8; font-weight: bold; cursor: pointer; transition: 0.2s; }
        .bc-tab-btn.active { background: #38bdf8; color: #0f172a; }
        .bc-tab-content { display: none; animation: fadeIn 0.3s; }
        ...
        /* LIGHTBOX XEM ẢNH */
        #bc-lightbox { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); z-index: 2147483647; justify-content: center; align-items: center; flex-direction: column; }
        #bc-lb-img { max-width: 95%; max-height: 85%; border-radius: 8px; box-shadow: 0 10px 30px rgba(0,0,0,0.8); object-fit:contain;}
    ...
    const runTool = () => {
        ...
        app.innerHTML = `
            ...
            <div class="bc-screen" id="sc-login" style="max-width:400px; padding:0;">
                <div class="bc-header" style="justify-content:flex-end; border:none; padding:15px; margin:0; background:transparent; box-shadow:none;">
                    <button class="bc-close-btn btn-close-app" style="position:absolute; right:15px; top:15px;">✕</button>
                </div>
        ...
        window.openLightbox = (src) => { $('bc-lb-img').src = src; $('bc-lightbox').style.display = 'flex'; };
        ...
        if (IS_MANAGER) {
            ...
            // Render DS Nhân viên (Tab Cấu Hình)
            const renderNV = () => {
            ...
            const renderStatList = (dateFilter) => {
                let filtered = dateFilter === "ALL" ? STAT_DATA : STAT_DATA.filter(r => r[0].startsWith(dateFilter));
                    ...
                    html += `
                        <div class="rp-card" onclick="document.getElementById('rp-det-${idx}').style.display = document.getElementById('rp-det-${idx}').style.display === 'block' ? 'none' : 'block'">
                            ...
                            <div class="rp-detail" id="rp-det-${idx}" onclick="event.stopPropagation();">
                                ...
                                ${renderImgGrid(imgToRoi)}
                                
                                <div style="margin:15px 0 10px;"><b>🌐 Đăng Bài:</b> ${linkDB ? `<a href="${linkDB}" target="_blank" class="rp-link">${linkDB}</a>` : 'Không có link'}</div>
                                ${renderImgGrid(imgDB)}
    ...
    return { name: "BC Truyền thông", icon: `<svg viewBox="0 0 24 24"><path fill="white" d="M3 3v18h18V3H3zm16 16H5V5h14v14zM7 10h2v7H7v-7zm4-3h2v10h-2V7zm4 6h2v4h-2v-4z"/></svg>`, bgColor: "#0284c7", action: runTool };
})
