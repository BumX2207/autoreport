((context) => {
    ...
    const MY_CSS = `
        ...
        .input-wrapper .bc-input { margin-bottom: 0; padding-right: 60px; }
    ...
    const runTool = () => {
        ...
        app.innerHTML = `
            ...
            <div class="bc-screen" id="sc-error" style="height:auto; max-height:none;">
                ...
                <div style="padding: 0 30px 40px; text-align:center;">
                    ...
                    <h2 style="color:#ef4444; margin-bottom:10px;" id="err-title">TỪ CHỐI TRUY CẬP</h2>
                    <p style="color:#94a3b8; font-size:14px; margin-bottom:25px; line-height: 1.5;" id="err-msg">Đang xử lý...</p>
                    <button class="bc-btn btn-danger btn-close-app" style="width: auto; padding: 10px 30px;">Đóng Tiện Ích</button>
                </div>
            </div>
            ...
            <div class="bc-screen" id="sc-report">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding: 8px 20px; background: rgba(0,0,0,0.3); border-bottom: 1px solid rgba(255,255,255,0.05); flex-shrink: 0;">
                    ...
                    <button class="bc-btn btn-danger" id="btn-nv-logout" style="padding:5px 12px; width:auto; font-size:11px; border-radius: 4px; box-shadow: 0 2px 5px rgba(0,0,0,0.2); margin: 0;">Đóng</button>
                </div>
        ...
        if($('btn-edit-folder')) $('btn-edit-folder').onclick = () => lockConfigInputs(false);
        if($('btn-edit-sheet')) $('btn-edit-sheet').onclick = () => lockConfigInputs(false);
        ...
        app.addEventListener('click', (e) => {
            if (e.target && e.target.classList.contains('rp-img')) {
                e.stopPropagation(); $('bc-lb-img').src = e.target.getAttribute('src'); $('bc-lightbox').style.display = 'flex';
                ...
                e.stopPropagation(); executeDownload(e.target.getAttribute('data-url'), e.target.getAttribute('data-filename'));
                ...
                let links =[];
                try { links = JSON.parse(linksStr); } catch(err) {}
                if (links.length > 0) {
                    if(!confirm(`Bắt đầu tải về ${links.length} ảnh?\n(Lưu ý: Trình duyệt có thể hỏi quyền "Cho phép Tải xuống nhiều tệp")`)) return;
                    links.forEach((l, idx) => {
        ...
        const handlePreview = (inputId, previewId) => {
            if($(inputId)) $(inputId).addEventListener('change', (e) => {
    ...
    // ===============================================================
    // HỆ THỐNG QUẢN LÝ QUỸ SIÊU THỊ (ĐA KHO - PHÂN QUYỀN CHUYÊN SÂU)
    // ===============================================================
    const FUND_SYSTEM = {
        loaded: {}, 
        ...
        renderUI: (containerId, isManager, currentUser, sheetId, keeper, trans) => {
            ...
            if (keeper) {
                ...
                if (kStr.startsWith('{')) {
                    try { keeperObj = JSON.parse(kStr); } catch(e) { keeperObj[actualShops[0]] = kStr; }
            ...
            if (isManager && actualShops.length > 1) {
                ...
                actualShops.forEach((s) => {
                    let safeS = String(s).replace(/[^a-zA-Z0-9]/g, '_');
                    html += `<div class="fund-sub-tab" data-target="fund-shop-${containerId}-${safeS}">🏬 Kho ${s}</div>`;
                });
                html += `</div>`;
            }
            ...
            const renderTransactionHTML = (tList, showShopBadge = false, isTabAll = false) => {
                ...
                tList.slice().reverse().forEach(t => {
                    ...
                    tHtml += `
                        <div class="fund-item-new" data-id="${t.id}" data-shop="${t.displayShop}">
                            <div class="fi-row-1">
                    ...
                    tHtml += `</div></div>
                            <div class="fi-row-2">
                                ...
                                <div class="fi-reason" title="${t.reason}">${shopBadge}${t.reason}</div>
                ...
                html += `
                    <div class="fund-dash">
                        <div class="fund-card-main">
                            <div style="color:#94a3b8; font-size:14px; font-weight:bold; text-transform:uppercase;">GRAND TOTAL - TẤT CẢ CÁC KHO</div>
                            <div class="fund-balance" style="color:#FFD700;">${FUND_SYSTEM.formatVNĐ(shopTrans['ALL'].balance)}</div>
                            <div style="display:flex; gap:10px; margin-top:15px; flex-wrap:wrap; border-top:1px dashed rgba(255,255,255,0.1); padding-top:15px;">
                `;
                actualShops.forEach(s => {
                    html += `
                        <div style="flex:1; min-width:80px; background:rgba(0,0,0,0.4); padding:10px; border-radius:8px; border:1px solid rgba(56,189,248,0.3); text-align:center;">
                            <div style="font-size:11px; color:#cbd5e1; margin-bottom:5px;">Kho ${s}</div>
                            <div style="font-size:14px; font-weight:bold; color:#38bdf8;">${FUND_SYSTEM.formatVNĐ(shopTrans[s].balance)}</div>
                        </div>
                    `;
                });
                html += `       </div>
            ...
            actualShops.forEach((s, idx) => {
                ...
                html += `<div id="fund-shop-${containerId}-${safeS}" class="fund-shop-content ${isActive}" style="${displayStyle}">`;
                
                html += `
                    <div class="fund-dash">
                        <div class="fund-card-main">
                            <div style="color:#94a3b8; font-size:14px; font-weight:bold; text-transform:uppercase;">Tổng Quỹ ${isManager && actualShops.length > 1 ? `Kho ${s}` : 'Hiện Tại'}</div>
                            <div class="fund-balance">${FUND_SYSTEM.formatVNĐ(shopTrans[s].balance)}</div>
                            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:15px; border-top:1px dashed rgba(255,255,255,0.1); padding-top:15px;">
                                <div><span style="font-size:12px; color:#94a3b8;">Thủ quỹ:</span></div>
                `;
                ...
                if (canAdd) {
                    ...
                    <div class="fund-actions">
                        <button class="fund-btn-thu btn-fund-add" data-shop="${s}" data-type="Thu">+ THU</button>
                        <button class="fund-btn-chi btn-fund-add" data-shop="${s}" data-type="Chi">- CHI</button>
                    </div>`;
                }

                html += `<div class="bc-sec-title">📝 LỊCH SỬ THU CHI</div><div class="fund-list">`;
                html += renderTransactionHTML(shopTrans[s].list, false, false);
                html += `</div></div>`; 
            });
            ...
            containerEl.querySelectorAll('.fund-item-new').forEach(item => {
                item.onclick = (e) => {
                    ...
                    if(transObj) {
                        const shopCode = item.getAttribute('data-shop');
                        FUND_SYSTEM.showDetailModal(transObj, shopCode);
                    }
                };
            });
        },
        ...
        if (IS_MANAGER) {
            ...
            app.addEventListener('delNV', (e) => { 
                ...
                renderNV(); 
                if (EDITING_EMP_INDEX === e.detail) { EDITING_EMP_INDEX = -1; resetEmpInputs(); }
            });
            ...
            $('btn-add-nv').onclick = () => {
                let s = $('inp-nv-shop').value.trim(), u = $('inp-nv-user').value.trim(), fn = $('inp-nv-fn').value.trim(), dob = $('inp-nv-dob').value.trim(), p = $('inp-nv-pass').value.trim(), role = $('inp-nv-role').value, grp = $('inp-nv-grp').value.trim();
                ...
                if (!/^[\p{L}\s]+$/u.test(fn)) return alert("Họ và tên chỉ được chứa chữ cái và khoảng trắng!");
                if (dob && !/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[0-2])\/\d{4}$/.test(dob)) return alert("Ngày sinh phải theo định dạng dd/mm/yyyy!");
                ...
                if (EDITING_EMP_INDEX > -1) {
                    ...
                    EDITING_EMP_INDEX = -1; resetEmpInputs();
                } else {
            ...
            const loadStatistics = async () => {
                try {
                    ...
                    if(json.status === 'success' && Array.isArray(json.data) && json.data.length > 1) {
                        ...
                        STAT_DATA.reverse(); 
                        renderTodaySummary(); updateFilters(); triggerRender();
                    } else {
            ...
            const renderTodaySummary = (empty = false) => {
                ...
                notReportedUsers.forEach(u => { listHtml += `<div class="emp-status-row"><span class="emp-name pending">${getEmpDisplayName(u)}</span><span style="color:#ef4444;">⏳</span></div>`; });
                reportedUsers.forEach(u => { listHtml += `<div class="emp-status-row"><span class="emp-name reported">${getEmpDisplayName(u)}</span><span style="color:#10b981;">✅</span></div>`; });
                if(allEmps.length === 0) listHtml += `<div style="text-align:center; padding: 10px; color:#94a3b8; font-size:13px;">Chưa có nhân viên nào trong danh sách.</div>`;
                listHtml += `</div>`;
            ...
            const updateDateDropdown = () => {
                ...
                let dates =[];
                if(selectedMonth === "ALL") dates =[...new Set(STAT_DATA.map(r => r.dateStr))];
                else dates =[...new Set(STAT_DATA.filter(r => r.monthStr === selectedMonth).map(r => r.dateStr))];
            ...
            const triggerRender = () => {
                let m = $('stat-month-filter').value, d = $('stat-date-filter').value, e = $('stat-emp-filter').value;
            ...
            const renderStatList = (filteredData, selectedEmp, selectedMonth, selectedDate) => {
                ...
                if (selectedEmp === "ALL") {
                    ...
                    let allFlyerImgs = [], allPostImgs =[], allLiveImgs =[], allPostLinks = [], allLiveLinks =[];
                    ...
                    finalHtml += `
                        ...
                        <div class="rp-card" style="border-color: #38bdf8; background: rgba(56, 189, 248, 0.05); margin-bottom: 25px;">
                            <div class="rp-header-row" onclick="document.getElementById('${uId}').style.display = document.getElementById('${uId}').style.display === 'block' ? 'none' : 'block'">
                                <div><b style="color:#38bdf8; font-size:15px;">📊 Số liệu: ${timeLabel}</b><div style="font-size:12px; color:#cbd5e1; margin-top:5px;">Số ngày báo cáo: ${activeDays.size}</div></div>
                            ...
                            <div class="rp-detail" id="${uId}" style="border-top-color:rgba(56,189,248,0.3); display:block;">
                                <div style="margin-bottom:15px;"><b style="color:#fff;">📄 Tổng Tờ Rơi Đã Phát: <span style="color:#FFD700; font-size:16px;">${totalFlyers}</span> tờ</b>${renderImgGrid(allFlyerImgs.filter(s=>s).join('|||'), true, `PhatToRoi_${selectedEmp}`)}</div>
                                <div style="margin-bottom:15px;"><b style="color:#fff;">🌐 Tổng Lượt Đăng Bài: <span style="color:#FFD700; font-size:16px;">${postCount}</span> lần</b><ul class="link-list">${allPostLinks.join('')}</ul>${renderImgGrid(allPostImgs.filter(s=>s).join('|||'), true, `DangBai_${selectedEmp}`)}</div>
                                <div style="margin-bottom:5px;"><b style="color:#fff;">🎥 Tổng Lượt Livestream: <span style="color:#FFD700; font-size:16px;">${liveCount}</span> lần</b><ul class="link-list">${allLiveLinks.join('')}</ul>${renderImgGrid(allLiveImgs.filter(s=>s).join('|||'), true, `Livestream_${selectedEmp}`)}</div>
                            </div>
                        </div>
                    `;
                }
                ...
                for(let date in grouped) {
                    finalHtml += `<div class="date-group-wrapper"><div class="date-group-title">📅 Nhật ký báo cáo ngày: ${date}</div><div class="date-group-content">`;
                    let totalToRoi = 0, allLinksDB =[], allLinksLive =[], allImgToRoi =[], allImgDB =[], allImgLive =[];

                    grouped[date].forEach(row => {
                        totalToRoi += parseInt(row.slToRoi) || 0;
                        if (row.linkDB) allLinksDB.push(`<li>renderMultipleLinks(row.linkDB)</li>`);
                        if (row.linkLive) allLinksLive.push(`<li>renderMultipleLinks(row.linkLive)</li>`);
                    ...
                    let combinedImgToRoi = allImgToRoi.filter(s => s).join('|||'), combinedImgDB = allImgDB.filter(s => s).join('|||'), combinedImgLive = allImgLive.filter(s => s).join('|||');
                    let uniqueIdSum = `rp-det-${date.replace(/\//g,'-')}-SUM`, safeDate = date.replace(/\//g,'-');
                        ...
                        finalHtml += `
                            <div class="rp-card">
                                ...
                                <div class="rp-detail" id="${uniqueId}">
                                    ...
                                    <div style="margin:15px 0 10px;"><b>🌐 Đăng Bài:</b>renderMultipleLinks(row.linkDB)</div>
                                    ${renderImgGrid(row.imgDB, false, `DangBai_${row.user}_${safeDate}`)}
                                    <div style="margin:15px 0 10px;"><b>🎥 Livestream:</b>renderMultipleLinks(row.linkDB)</div>
                        ...
                        try {
                            ...
                            if (bossData.status === 'success') {
                                ...
                                if (bossData.employees && bossData.employees !== "[]") {
                                    ...
                                    // FIX SINH NHẬT: Ưu tiên lấy Shop thực tế phòng trường hợp myEmp bị null
            ...
            const renderNLNV = async (mode) => {
                ...
                try {
                    ...
                    if (mode === 'overview') { container.innerHTML = LOCAL_BI_ENGINE.getNLNVReport(latestDataCache, mockConfigList, mgrConfig, staffNameInBI, shopIdx, daysPassed, daysInMonth, latestDate); } 
                    else if (mode === 'daily') { container.innerHTML = LOCAL_BI_ENGINE.getNLNVDailyReport(historyCache, mockConfigList, mgrConfig, staffNameInBI, shopIdx, daysPassed, daysInMonth); }
                                    ...
                                    html += `
                                        <div class="rp-card" style="margin-bottom: 10px;">
                                            ...
                                            <div class="rp-detail" id="${uniqueId}">
                                                ...
                                                <div style="margin:15px 0 10px;"><b>🌐 Lượt Đăng/Share Bài:</b> renderMultipleLinks(row.linkDB)</div>
                                                ${renderImgGrid(row.imgDB, false, `DangBai_${date.replace(/\//g,'')}_${idx+1}`)}
                                                
                                                <div style="margin:15px 0 10px;"><b>🎥 Livestream:</b> renderMultipleLinks(row.linkLive)</div>
            ...
            $('btn-submit-report').onclick = async () => {
                ...
                try {
                    ...
                    // Gom nhiều link lại và cách nhau bởi dấu xuống dòng (\n)
                    ...
                    if(JSON.parse(response).status === 'success') {
                        ...
                        if($('inp-toroi-sl')) $('inp-toroi-sl').value = '';
