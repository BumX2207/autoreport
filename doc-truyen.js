    ...
    const runTool = async () => {
        ...
        if (!app) {
            ...
            const loadVoices = () => {
                ...
                const sel = $('sel-voice');
                sel.innerHTML = `<option value="-1">Mặc định (Google/Chị Google)</option>`;
                availableVoices.forEach((v, i) => {
                    sel.innerHTML += `<option value="${i}">${v.name}</option>`;
                });
            };
        ...
        const handleChapterFinished = () => {
            isReading = false;
            saveCloudHistory(); // Lưu khi hết chương
